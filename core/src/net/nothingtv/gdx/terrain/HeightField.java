...
/** This is a test class, showing how one could implement a height field. See also {@link SimpleTerrain}. Do not expect this to be
...
public class HeightField implements Disposable {
	...
	public float getHeightAt(int x, int y) {
		// clamp x and y to the height field
		if (x < 0) x = 0;
		if (x >= width) x = width - 1;
		if (y < 0) y = 0;
		if (y >= height) y = height - 1;
		return data[y * width + x] * magnitude.y;
	}
